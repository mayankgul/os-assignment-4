...
extern int sys_ps(void); // Making sys_ps declaration
...
static int (*syscalls[])(void) = {
    ...
    [SYS_mkdir] sys_mkdir,
    [SYS_close] sys_close,
    [SYS_ps] sys_ps, // Means at the index of SYS_ps that is 22 we are storing the function pointer sys_ps
...
void syscall(void)
{
    ...
    {
        proc->syscall_count++; // Increment counter
        ret = syscalls[num]();
